...
class WorldCupApp:
    ...
    def _show_match_screen(self) -> None:
        ...
        try:
            current_images = tuple(
                load_image_for_display(entry.path)
                for entry in tournament.current_match.participants
            )
        except Exception as error:
    ...
    def _show_champion_screen(self) -> None:
        ...
        try:
            image = load_image_for_display(champion.path)
        except Exception as error:
            self._abort_for_missing_image(error)
            return
        ...
        ttk.Entry(outer, textvariable=path_var, state="readonly").pack(
            fill="x", pady=(12, 12)
        )
